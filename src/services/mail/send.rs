...
fn get_from() -> Result<Mailbox, AppError> {
    let from_adress = Address::new(load_env_var("MAIL_USER")?, load_env_var("MAIL_DOMAIN")?)
        .map_err(|_| AppError {
            code: Code::Internal,
...
pub fn send_email(to_adress: String, subject: String, body: String) -> Result<String, AppError> {
    ...
    match get_transport()?.send(&message) {
        ...
        Err(error) => {
            ...
            Err(AppError {
                code: Code::Internal,
                message: String::from("failed to send email"),
            })
        }
    }
}
...
mod tests {
    ...
    fn test_send_email() {
        ...
        let response = send_email(
            load_env_var("MAIL_USER_ADRESS").unwrap(),
            String::from("email test"),
            String::from("<div>This is a teste</div>"),
        )
        .unwrap();
